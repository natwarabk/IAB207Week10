...
def show(id):
    ...
    if not destination:
       abort(404)
    return render_template('destinations/show.html', destination=destination, comment_form=comment_form)
...
def comment(id):  
    ...
    if form.validate_on_submit():  
      ...
      try:
          ...
          db.session.rollback()
          flash('Error: Could not add your comment. Please try again.', 'danger')
